 ...
 */
public class UserBasedDivRankALSExample {
	public static void main(String[] args) throws IOException {
		String divUserPath = args[0];
		String itemPath = args[1];
		String trainDataPath = args[2];
		String divTestDataPath = args[3];
		String featurePath = args[4];

		FastUserIndex<Long> divUserIndex = SimpleFastUserIndex.load(divUserPath, lp);
		FastItemIndex<Long> itemIndex = SimpleFastItemIndex.load(itemPath, lp);


		FastPreferenceData<Long, Long> trainData = SimpleFastPreferenceData.load(trainDataPath, lp, lp, ddp, divUserIndex, itemIndex);
		FastPreferenceData<Long, Long> divTestData = SimpleFastPreferenceData.load(divTestDataPath, lp, lp, ddp, divUserIndex, itemIndex);
		...
		Recommender<Long, Long> divRecommender = new MFRecommender<>(divUserIndex, itemIndex, divfactorization);
		Recommender<Long, Long> recommender = new MFRecommender<>(divUserIndex, itemIndex, factorization);
		...
		////////////////////////////////
		Set<Long> targetUsers = divTestData.getUsersWithPreferences().collect(Collectors.toSet());
		RecommendationFormat<Long, Long> format = new SimpleRecommendationFormat<>(lp, lp);
		Function<Long, IntPredicate> filter = FastFilters.notInTrain(trainData);
		int maxLength = 20;
		RecommenderRunner<Long, Long> runner = new FastFilterRecommenderRunner<>(divUserIndex, itemIndex, targetUsers, format, filter, maxLength);
		System.out.println("Running Div RankALS");
		runner.run(divRecommender, "divrankals");
